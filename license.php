...
function getReusableDevRequest(mysqli $conn, string $serverIP, string $resource, string $license = ''): ?array
{
    // Reuse only the exact same server/resource/key combination so retries do not create duplicates.
    $sql = "\n        SELECT *\n        FROM scriptforge_dev_requests\n        WHERE server_ip = ?\n        AND resource_name = ?\n";
    $types = 'ss';
    $params = [$serverIP, $resource];
    ...
    $sql .= "        AND status IN ('pending', 'approved', 'denied', 'revoked')\n        ORDER BY id DESC\n        LIMIT 1\n    ";
...
function createDevRequest(
    ...
    return getReusableDevRequest($conn, $serverIP, $resource, $license);
}
...
if (isPlaceholderLicenseKey($license, $placeholderKeys)) {
    ...
    $devRequest = getReusableDevRequest($conn, $serverIP, $resource, $license);
